...
import static org.apache.commons.imaging.formats.tiff.constants.TiffConstants.BIG_TIFF_ENTRY_MAX_VALUE_LENGTH;
...
public class TiffReader extends BinaryFileParser {
    ...
    private TiffHeader readTiffHeader(final InputStream is) throws ImageReadException, IOException {
        ...
        tiffVersion = read2Bytes("tiffVersion", is, "Not a Valid TIFF File", getByteOrder());
        ...
        {
            final int byteSize = read2Bytes("byteSize", is, "Not a Valid TIFF File", getByteOrder());
            if (byteSize != 8) {
                throw new ImageReadException("Custom rda-tiff only supports bigtiff: (0x002B)(43) with offset bytesize of 8" + byteSize);
            }
            final int constant = read2Bytes("constant", is, "Not a Valid TIFF File", getByteOrder());
            if (constant != 0) {
                throw new ImageReadException("Custom rda-tiff only supports bigtiff: (0x002B)(43) with offset bytesize of 8" + byteSize);
            }
            offsetToFirstIFD =
                    0xFFFFffffL & read8Bytes("offsetToFirstIFD", is, "Not a Valid TIFF File", getByteOrder());
        }
        ...
        try (InputStream is = byteSource.getInputStream()) {
            ...
            try {
                if(tiffVersion == TiffConstants.TIFF_CLASSIC)
                    ...
                    entryCount = (int)read8Bytes("DirectoryEntryCount", is, "Not a Valid TIFF File", getByteOrder());
            ...
            for (int i = 0; i < entryCount; i++) {
                ...
                if(tiffVersion == TiffConstants.TIFF_CLASSIC)
                {
                    tag = read2Bytes("Tag", is, "Not a Valid TIFF File", getByteOrder());
                    type = read2Bytes("Type", is, "Not a Valid TIFF File", getByteOrder());
                ...
                else
                {
                    tag = read2Bytes("Tag", is, "Not a Valid TIFF File", getByteOrder());
                    type = read2Bytes("Type", is, "Not a Valid TIFF File", getByteOrder());
                    count = 0xFFFFffffL & read8Bytes("Count", is, "Not a Valid TIFF File", getByteOrder());
                ...
                final byte[] value;
                final int maxLength = tiffVersion == TiffConstants.TIFF_CLASSIC ? TIFF_ENTRY_MAX_VALUE_LENGTH : BIG_TIFF_ENTRY_MAX_VALUE_LENGTH;
            ...
            if(tiffVersion == TiffConstants.TIFF_CLASSIC) {
                nextDirectoryOffset = 0xFFFFffffL & read4Bytes("nextDirectoryOffset", is,
                        "Not a Valid TIFF File", getByteOrder());
            }
            else
            {
                nextDirectoryOffset = 0xFFFFffffL & read8Bytes("nextDirectoryOffset", is,
                        "Not a Valid TIFF File", getByteOrder());
            }
