...
import com.maxar.rda.imaging.PixelDensity;
import com.maxar.rda.imaging.common.BinaryOutputStream;
import com.maxar.rda.imaging.common.PackBits;
import com.maxar.rda.imaging.common.RationalNumber;
import com.maxar.rda.imaging.common.itu_t4.T4AndT6Compression;
import com.maxar.rda.imaging.common.mylzw.MyLzwCompressor;
...
public abstract class TiffImageWriterBase {
    ...
    protected static long imageDataPaddingLength(final long dataLength) {
    ...
    protected TiffOutputSummary validateDirectories(final TiffOutputSet outputSet)
        ...
        if (interoperabilityDirectory == null
            ...
            if (interoperabilityDirectoryOffsetField == null) {
                ...
                //todo: don't know if these need to be bigtiff long8 offsets or not set to consistent with root dir for now
                if(rootDirectory.getVersion() == TiffConstants.TIFF_CLASSIC)
                {
                ...
                {
                    interoperabilityDirectoryOffsetField =
                            TiffOutputField.createOffsetFieldLong8(
                                    ExifTagConstants.EXIF_TAG_INTEROP_OFFSET,
                                    byteOrder);
                }
        ...
        if (exifDirectory == null && exifDirectoryOffsetField != null) {
            ...
            if (exifDirectoryOffsetField == null) {
                ...
                {
                    exifDirectoryOffsetField = TiffOutputField.createOffsetFieldLong8(
                            ExifTagConstants.EXIF_TAG_EXIF_OFFSET, byteOrder);
                }
        ...
        if (gpsDirectory == null && gpsDirectoryOffsetField != null) {
            ...
            if (gpsDirectoryOffsetField == null) {
                ...
                {
                    gpsDirectoryOffsetField = TiffOutputField.createOffsetFieldLong8(
                            ExifTagConstants.EXIF_TAG_GPSINFO, byteOrder);
                }
    ...
    public void writeImage(final BufferedImage src, final OutputStream os, Map<String, Object> params)
        ...
        int compression = TiffConstants.TIFF_COMPRESSION_LZW; // LZW is default
        ...
        if (params.containsKey(ImagingConstants.PARAM_KEY_COMPRESSION)) {
            ...
            if (params.containsKey(TiffConstants.PARAM_KEY_LZW_COMPRESSION_BLOCK_SIZE)) {
                final Object bValue =
                    params.get(TiffConstants.PARAM_KEY_LZW_COMPRESSION_BLOCK_SIZE);
                ...
                params.remove(TiffConstants.PARAM_KEY_LZW_COMPRESSION_BLOCK_SIZE);
            }
        }
        final HashMap<String, Object> rawParams = new HashMap<>(params);
        params.remove(TiffConstants.PARAM_KEY_T4_OPTIONS);
        params.remove(TiffConstants.PARAM_KEY_T6_OPTIONS);
        if (!params.isEmpty()) {
        ...
        if (compression == TiffConstants.TIFF_COMPRESSION_CCITT_1D
                || compression == TiffConstants.TIFF_COMPRESSION_CCITT_GROUP_3
        ...
        } else if (compression == TiffConstants.TIFF_COMPRESSION_CCITT_GROUP_3) {
            final Integer t4Parameter = (Integer) rawParams.get(TiffConstants.PARAM_KEY_T4_OPTIONS);
            if (t4Parameter != null) {
        ...
        } else if (compression == TiffConstants.TIFF_COMPRESSION_CCITT_GROUP_4) {
            final Integer t6Parameter = (Integer) rawParams.get(TiffConstants.PARAM_KEY_T6_OPTIONS);
            if (t6Parameter != null) {
        ...
        } else if (compression == TiffConstants.TIFF_COMPRESSION_PACKBITS) {
            for (int i = 0; i < strips.length; i++) {
                strips[i] = new PackBits().compress(strips[i]);
            }
        } else if (compression == TiffConstants.TIFF_COMPRESSION_LZW) {
            for (int i = 0; i < strips.length; i++) {
        ...
        } else if (compression == TiffConstants.TIFF_COMPRESSION_UNCOMPRESSED) {
            // do nothing.
        } else {
            throw new ImageWriteException(
    ...
    protected void writeImageFileHeader(final BinaryOutputStream bos, final int version)
            throws IOException {
        final int offsetToFirstIFD = version == TiffConstants.TIFF_CLASSIC ? TiffConstants.TIFF_HEADER_SIZE : TiffConstants.BIG_TIFF_HEADER_SIZE;

        writeImageFileHeader(bos, version, offsetToFirstIFD);
    }
    ...
    protected void writeImageFileHeader(final BinaryOutputStream bos, final int version,
        ...
        {
            bos.write2Bytes(TiffConstants.TIFF_CLASSIC); // tiffVersion
            bos.write4Bytes((int) offsetToFirstIFD);
        }
        else
        {
            bos.write2Bytes(TiffConstants.TIFF_BIGTIFF); // 0x002B version = BigTIFF
            bos.write2Bytes(8); // 0x0008 bytesize of offsets
            bos.write2Bytes(0); // 0x0000 constant
            bos.write8Bytes(offsetToFirstIFD);
        }
