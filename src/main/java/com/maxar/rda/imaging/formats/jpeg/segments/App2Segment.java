...
import static com.maxar.rda.imaging.common.BinaryFunctions.readAndVerifyBytes;
...
public class App2Segment extends AppnSegment implements Comparable<App2Segment> {
    ...
    public App2Segment(final int marker, int markerLength, final InputStream is2)
        ...
        if (BinaryFunctions.startsWith(getSegmentData(),
                JpegConstants.ICC_PROFILE_LABEL)) {
            final InputStream is = new ByteArrayInputStream(getSegmentData());

            BinaryFunctions.readAndVerifyBytes(is, JpegConstants.ICC_PROFILE_LABEL,
                    "Not a Valid App2 Segment: missing ICC Profile label");

            curMarker = BinaryFunctions.readByte("curMarker", is, "Not a valid App2 Marker");
            numMarkers = BinaryFunctions.readByte("numMarkers", is, "Not a valid App2 Marker");

            markerLength -= JpegConstants.ICC_PROFILE_LABEL.size();
            markerLength -= (1 + 1);

            iccBytes = BinaryFunctions.readBytes("App2 Data", is, markerLength, "Invalid App2 Segment: insufficient data");
        } else {
