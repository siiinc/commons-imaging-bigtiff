 ...
 */
package com.maxar.rda.imaging.common;
...
public final class ByteConversions {
    ...
    public static byte[] toBytes(final long[] values, final ByteOrder byteOrder) {
        return toBytes(values, 0, values.length, byteOrder);
    }

    private static byte[] toBytes(final long[] values, final int offset, final int length, final ByteOrder byteOrder) {
        final byte[] result = new byte[length * Long.BYTES];
        for (int i = 0; i < length; i++) {
            toBytes(values[offset + i], byteOrder, result, i * Long.BYTES);
        }
        return result;
    }
    ...
    private static long[] toLongs(final byte[] bytes, final int offset,
                                  final int length, final ByteOrder byteOrder) {
        final long[] result = new long[length / Long.BYTES];
        for (int i = 0; i < result.length; i++) {
            result[i] = toLong(bytes, offset + Long.BYTES * i, byteOrder);
        }
        return result;
    }

    /*
        JR added to/from unsigned long
     */


//    private static byte[] toByteArrayUnsigned(BigInteger bi) {
//        byte[] extractedBytes = bi.toByteArray();
//        if(extractedBytes.length < 8)
//        {
//            extractedBytes = toBytes(bi.longValue(), ByteOrder.BIG_ENDIAN);
//        }
//        int skipped = 0;
//        boolean skip = true;
...
//    }
//
//    private static byte[] toByteArrayLittleEndianUnsigned(BigInteger bi) {
//        byte[] extractedBytes = toByteArrayUnsigned(bi);
//        for(int i = 0; i < extractedBytes.length; i++)
//        {
//            byte temp = extractedBytes[i];
//            extractedBytes[i] = extractedBytes[extractedBytes.length - i - 1];
//            extractedBytes[extractedBytes.length - i - 1] = temp;
//        }
//        return extractedBytes;
//    }
//
//    public static byte[] toBytes(final UInt64 value, final ByteOrder byteOrder) {
//        final byte[] result = new byte[UInt64.BYTES];
//        toBytes(value, byteOrder, result, 0);
//        return result;
//    }
//
//    public static byte[] toBytes(final UInt64[] values, final ByteOrder byteOrder) {
//        return toBytes(values, 0, values.length, byteOrder);
//    }
//
//    private static byte[] toBytes(final UInt64[] values, final int offset, final int length, final ByteOrder byteOrder) {
//        final byte[] result = new byte[length * UInt64.BYTES];
//        for (int i = 0; i < length; i++) {
//            toBytes(values[offset + i], byteOrder, result, i * UInt64.BYTES);
//        }
//        return result;
//    }
//
//    private static void toBytes(final UInt64 value, final ByteOrder byteOrder, final byte[] result, final int offset) {
//
//        byte[] rep;
//        if (byteOrder == ByteOrder.BIG_ENDIAN) {
//            rep = toByteArrayUnsigned(value.value());
//        } else {
//            rep = toByteArrayLittleEndianUnsigned(value.value());
//        }
//        System.arraycopy(rep, 0, result, offset, rep.length);
//    }
//
//
//
//    public static UInt64 toUInt64(final byte[] bytes, final ByteOrder byteOrder) {
//        return toUInt64(bytes, 0, byteOrder);
//    }
//
//    private static UInt64 toUInt64(final byte[] bytes, final int offset, final ByteOrder byteOrder) {
//        if (byteOrder == ByteOrder.BIG_ENDIAN) {
//            return new UInt64(new BigInteger(bytes));
//        } else {
//            byte[] reversed = new byte[bytes.length];
//            System.arraycopy(bytes, 0, reversed, 0, reversed.length);
//            for(int i = 0; i < reversed.length / 8; i++)
//            {
//                byte temp = reversed[i];
//                reversed[i] = reversed[reversed.length - i - 1];
//                reversed[reversed.length - i - 1] = temp;
//            }
//            return new UInt64(new BigInteger(reversed));
//        }
//    }
//
//    public static UInt64[] toUInt64s(final byte[] bytes, final ByteOrder byteOrder) {
//        return toUInt64s(bytes, 0, bytes.length, byteOrder);
//    }
//
//    private static UInt64[] toUInt64s(final byte[] bytes, final int offset,
//                                  final int length, final ByteOrder byteOrder) {
//        final UInt64[] result = new UInt64[length / UInt64.BYTES];
//        for (int i = 0; i < result.length; i++) {
//            result[i] = toUInt64(bytes, offset + Long.BYTES * i, byteOrder);
//        }
//        return result;
//    }

    /*
    End of long handling
     */
